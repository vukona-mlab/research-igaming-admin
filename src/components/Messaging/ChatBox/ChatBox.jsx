...
const ChatBox = ({
    ...
    socketRef.current.on("messages-read", (data) => {
      if (data.chatId === chatId) {
        ...
        setMessages(prev => prev.map(msg => {
          ...
          if (msg.senderId === currentClientId && !msg.readBy?.includes(data.userId)) {
  ...
  const markMessagesAsRead = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/adminChats/${chatId}/read`,
        {
          method: "PUT",
          headers: {
            Authorization: token.startsWith("Bearer ") ? token : `Bearer ${token}`,
    ...
    if (currentChat.participants && Array.isArray(currentChat.participants)) {
      ...
      if (otherParticipant) {
        const structuredData = {
          ...currentChat,
          metadata: {
            target: typeof otherParticipant === 'object' ? otherParticipant : {
              id: otherParticipant,
  ...
  if (loading && messages.length === 0) return <div className="loading">Loading messages...</div>;
